                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($guru as $index => $guru1)
                                    <tr>
                                        <td>{{ $guru->firstItem() + $index }}</td>
                                        <td>{{ $guru1->nama_guru }}</td>
                                        <td class="text-left">
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('guru.destroy', $guru1->id) }}" method="POST">
                                                <a href="{{ route('guru.edit', $guru1->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                                @csrf
                                                @method('DELETE')
